...
/**
 * Toggle + Amount/Currency — shown before the comment field. Split from
 * the Unit escalation select (below) so forms can place the two pieces
 * on either side of the comment, per the requested form order:
 * Type → Transactional toggle+amount → Comment → Escalate to unit.
 */
export function TransactionalToggle({
  isTransactional,
  onToggle,
  amount,
  onAmountChange,
